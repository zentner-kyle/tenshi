...
// TODO(kzentner): Don't duplicate this from ndl3.h
const NDL3_PORT_UNRELIABLE = 1;
...
const CONFIG_PORT = typpo.get_const('NDL3_CONFIG_PORT');
const FAST_PORT = typpo.get_const('NDL3_FAST_PORT');
...
var Radio = function(address, serportObj) {
  ...
  call('NDL3_open', this.net, CONFIG_PORT);
  call('NDL3_open', this.net, FAST_PORT);
  ...
  this.on('send_config', send_config.bind(this));
  this.on('send_fast', send_fast.bind(this));
  this.on('data', recv_L2.bind(this));

  call('NDL3_setopt', CONFIG_PORT, NDL3_PORT_UNRELIABLE);
  call('NDL3_setopt', FAST_PORT, NDL3_PORT_UNRELIABLE);
...
function send_config(buf) {
  /* jshint validthis: true */
  if (typeof buf === 'string') {
    buf = buffer.Buffer(buf);
  }
  send(this.net, CONFIG_PORT, buf);
}

function send_fast(object) {
  /* jshint validthis: true */
  var buf = ubjson.encode(object);
  send(this.net, FAST_PORT, buf);
}
